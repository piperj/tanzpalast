...
The hamburger menu lists the 6 hardcoded collections (International, Smooth, Latin, Rhythm, Club, Showcase). Tapping a collection filters the catalog to dances where at least one video carries a matching tag (e.g. `international`, `latin`). Tags drive the filter — there is no `collection:` field in the data.
...
```
┌─────────────────────────────┐
│ ≡   Tanzpalast · International  🏛│  ← sticky header; active collection shown inline
├─────────────────────────────┤
│ ┌─────────────────────────┐ │
...
**Hamburger (≡)**: slides out a panel listing the 6 hardcoded collections. Tapping a collection closes the panel, updates the header, and filters cards to those whose videos are tagged with that collection.
...
- Newly auto-inserted stubs carry `new` as the first tag — permanent, powers a filter for recently-added videos; never removed
...
| `tags`        | string[] | always      | Collection tag + descriptive keywords; `new` tag = recently added, permanent (never removed) |
...
|--------------|------------|--------|
| **International** | `international` | Waltz, Tango, Viennese Waltz, Foxtrot, Quickstep |
