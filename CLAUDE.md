...
**Planned (not yet implemented):** The JSON will be committed to the repo (un-ignore `data/tanzpalast-data.json` in `.gitignore`) and served by GitHub Pages at the same relative path as local dev. `DATA_URL` in `index.html` will become the constant `'data/tanzpalast-data.json'` — no environment detection needed. On GitHub, a build workflow will rebuild and commit the JSON whenever `tanzpalast.yaml` changes.

The canonical schema and field definitions are in `SPEC.md`. The YAML source file is `tanzpalast.yaml`.

## Current State (2026-04-06)

- Steps 0–6 complete and committed to `main` (local repo only — GitHub not yet set up)
...
- `data/tanzpalast-data.json` has 10 dances; Showcase collection active (6th); still gitignored
- `index.html` still uses env-detection + Google Drive `DATA_URL` — not yet simplified
...
**Next:** Create GitHub repo → push → enable GitHub Pages → add build workflow → un-ignore JSON → simplify `DATA_URL` in `index.html`.
